 ...
 * @param {Object} props.data - todo datas
 ...
 * @param {Boolean} props.data.isCompleted - is todo completed?
...
export default function Todo({ data, setRefresh }) {
  ...
  const handleUpdateTodo = async e => {
    try {
      const { data: res } = await request(`/api/todos/${data.id}`, {
        method: 'PUT',
  ...
  // handle update todo is completed
  const handleUpdateTodoIsCompleted = async e => {
    try {
      setDatas({ ...datas, isCompleted: !datas.isCompleted })
      const { data: res } = await request(`/api/todos/${data.id}`, {
        method: 'PATCH',
        data: { isCompleted: !datas.isCompleted },
        token: loginState.token
      })
      setRefresh(Math.random())
    } catch (err) {
  ...
  const handleDeleteTodo = async e => {
    try {
      const { data: res } = await request(`/api/todos/${data.id}`, { method: 'DELETE', token: loginState.token })
      setRefresh(Math.random())
    } catch (err) {
      console.log(err)
    }
      ...
      <div className="py-2 flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <label
            ...
            className={`text-lg transition duration-100 ${datas.isCompleted ? 'line-through text-gray-500' : ''}`}
          >
            {data.title}
          </label>
        </div>
